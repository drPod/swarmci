...
async def import_github(url: str):
    ...
    if use_nango:
        ...
        issue = await nango.action(
            connection, "get-issue", {"owner": owner, "repo": repo, "issue_number": int(number)}
        )
    else:
...
async def export_github(repository: str, title: str, body: str):
    ...
    return await nango.action(
        connection, "create-issue", {"owner": owner, "repo": repo, "title": title, "body": body}
    )
