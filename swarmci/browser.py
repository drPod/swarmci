...
class ReplayError(RuntimeError):
    def __init__(self, step, action, cause):
        super().__init__(f"Replay step {step + 1} ({action.label or action.kind}) failed: {cause}")
        self.step = step
...
class BrowserSession:
        ...
        try:
            ...
            def document_response(response):
                if response.request.is_navigation_request() and response.frame == self.page.main_frame:
                    self.document_response = {"url": response.url, "status": response.status}

            self.page.on("response", document_response)
    ...
    async def replay(self, path, annotate=False):
        ...
        for i, raw in enumerate(path):
            ...
            try:
                ...
                raise ReplayError(i, a, exc) from exc
    ...
    async def checks(self, action=None):
        ...
        for rule in self.target.assertions:
            ...
            if rule.kind == "document_status":
                response = self.document_response
                if not response or response["url"].split("#")[0] != self.page.url.split("#")[0]:
                    # A SPA route has no document response of its own. Do not
                    # invent a successful check for it or reuse another route's status.
                    continue
                results.append(
                    {
                        "name": rule.name,
