...
# Prefer unique semantic addresses captured before the action changes the DOM.
# Positional XPath remains the fallback for controls without a stable address.
REPLAY_SELECTORS = r"""entries => Object.fromEntries(entries.map(([index, xpath]) => {
 const e = document.evaluate(xpath, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
 if (!e) return [index, 'xpath='+xpath];
 ...
 for (const key of ['data-testid','aria-label','name']) {
   if (!e.getAttribute(key)) continue;
   const s = e.tagName.toLowerCase()+attr(key,e.getAttribute(key));
   if (unique(s)) return [index,s];
 }
 const link = e.closest('a[href]');
 if (link) {
   const base='a'+attr('href',link.getAttribute('href'));
   if (unique(base)) {
     if (e===link) return [index,base];
     const matches=[...link.querySelectorAll(e.tagName.toLowerCase())];
     if(matches.length===1) return [index,base+' '+e.tagName.toLowerCase()];
   }
 }
 if (e.id && !/radix|headlessui|:r|^:/.test(e.id)) {
   const s='#'+CSS.escape(e.id); if(unique(s)) return [index,s];
 }
 return [index,'xpath='+xpath];
}))"""
...
async def explore_bu(
    ...
    async def before(a):
        ...
        selectors.clear()
        addresses = await session.page.evaluate(
            REPLAY_SELECTORS, [[index, node.xpath] for index, node in state.dom_state.selector_map.items()]
        )
        selectors.update({int(index): value for index, value in addresses.items()})
    ...
    async def after(a):
        ...
        if last.model_output:
            for raw in last.model_output.action:
                ...
                if element and params.get("index") is not None and params["index"] not in selectors:
