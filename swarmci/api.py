...
from swarmci.integrations_api import install_integrations
from swarmci.models import RunConfig
from swarmci.narration import install_narration
from swarmci.presentation import enrich_snapshot
from swarmci.runner import Coordinator
from swarmci.storage import storage_status, watch_storage
...
async def lifespan(app):
    initialize()
    store.recover()
    storage_task = asyncio.create_task(watch_storage(coordinator, narration))
    yield
    storage_task.cancel()
    await asyncio.gather(storage_task, return_exceptions=True)
    tasks = list(coordinator.tasks.values()) + list(narration.tasks.values())
    for task in tasks:
        task.cancel()
    if tasks:
...
install_integrations(app, store)
narration = install_narration(app, store)
...
async def start(config: RunConfig):
    if not storage_status()["accepting_runs"]:
        raise HTTPException(503, "Low disk space. Free storage before starting another exploration.")
...
async def snapshot(run: str):
    try:
        return enrich_snapshot(store.snapshot(run))
    except KeyError:
