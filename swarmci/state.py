...
OBSERVE = r"""() => {
 ...
 const selector = e => {
   ...
   if(link){
     const base='a[href='+JSON.stringify(link.getAttribute('href'))+']';
     if(document.querySelectorAll(base).length===1){
       if(e===link) return base;
       if(link.querySelectorAll(e.tagName.toLowerCase()).length===1) return base+' '+e.tagName.toLowerCase();
     }
   }
 ...
 type:e.type,editable:e.isContentEditable,
