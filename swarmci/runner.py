...
from swarmci.browser import BrowserSession, ReplayError
...
class Coordinator:
        ...
        async with BrowserSession(target, folder, config.cloud_browser) as session:
            with span(
                ...
                try:
                    ...
                    end = exc.step + 1 - len(target.setup)
                    if end > 0:
                        self.store.replay_failure(run, [a.model_dump() for a in path[:end]])
                    raise
            ...
            if config.engine == "fixture":
                ...
                if payload.get("first_control"):
                    control = next(
                        (c for c in state["controls"] if control_key(c) == payload["first_control"]), None
                    )
                    if control is None or control.get("disabled"):
                        raise ValueError("Assigned control is no longer available after restore")
                    first = Action(kind="click", selector=control["selector"], label="Click " + control["label"])
                    await session.act(first)
                    if not await record(first):
                        return
                await explore_bu(
                    session,
                    target,
    ...
    async def branch(self, run, config, state, path, worker):
        if len(path) >= config.max_depth:
            ...
            return
        # Queued alternatives must not consume the execution budget: later, deeper
        # discoveries need to compete with the initial shallow frontier.
        count = self.store.attempted_job_count(run)
        ...
        if config.engine == "fixture":
            ...
            attempts = self.store.exploration_attempts(run, state["screen_key"])
            variants = exploration_frontier(state, config.target, attempts)
            if variants and config.use_gemma:
                try:
                    selected = {v["control_key"] for v in variants}
                    focused_state = {**state, "controls": [c for c in state["controls"] if control_key(c) in selected]}
                    suggestions = await objectives(focused_state, config.target, count=len(variants), gemma=True)
                    for variant, suggestion in zip(variants, suggestions):
                        variant["objective"] += " Additional workflow variation: " + suggestion
                except Exception as exc:
                    self.store.event(run, "model.error", {"model": "gemma", "message": str(exc)})
            if not variants and not state["controls"]:
                options = await objectives(state, config.target)
                variants = [{"objective": o, "priority": len(path)} for o in options]
        planned = []
